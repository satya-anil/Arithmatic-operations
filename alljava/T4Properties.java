 ...
 * inherited directy or indirectly by the <code>SQLMXDriver, SQLMXDataSource,
 * SQLMXConnectionPooledDataSource</code>
 ...
 * <code>–Dt4sqlmx.properties</code> option.
 * </p>
 * <p>
...
public class T4Properties {
	...
	private boolean commitInAutoCommitOn_;  //added for R30

	private String KANJI_;  //added for R30

	private String KSC5601_; //added for R30
	...
	// Default catalog NEO - AM 6/27/2006
	//Now Default catlog is changed to NONSTOP_SYSTEM_NSK - R30
//	static final String DEFAULT_CATALOG = "NONSTOP_SYSTEM_NSK";
	...
	//Added for stmt cache - R30
	private String mfuStatementCache = "YES";

	// Added new MFC property enableMFC
	private String enableMFC = "ON";
	
	//RFE autoCommit changes
	private String autoCommit_ = "ON";
  
  
	// Added property to support R2.3 date format Soln. 10-130717-8108
	private String oldDateFormat = "OFF";
  
    //R3.1 chages -- Added for R3.1
    private long queryExecuteTime;
    //R3.2 changes -- port from neo
	...
	//Execute Batch with valid rows affected
	...
	private String useExternalTransaction="NO";  //Added for commit work RFE
	...
	//R3.1 changes -- Added to get tge separate log file for SlowQueries
	static String t4SlowQueryGlobalLogFile = null;
	static Logger t4SlowQueryGlobalLogger = null;
	static FileHandler t4SlowQueryGlobalLogFileHandler = null;
	private String t4QueryExecuteLogFile_;
	FileHandler t4SlowQueryLogFileHandler_;
	 //Soln 10-120724-4205
	 private String dbMetaDataforDBTools="OFF";
	 //Sol 10-190130-9260
	 private String closeConnectionUponStatementCancel = "YES";

	// sol. 10-060719-7809 R3.0
	void initializeLogging(Properties p) {
		...
		try {
			...
			if (userSpecifiedLogLevel == null) {
				String temp = System.getProperty("t4sqlmx.T4LogLevel");
				if (temp != null) {
					userSpecifiedLogLevel = Level.parse(temp);
				} else {
			...
			//
			if (userSpecifiedLogFile != null
					|| (userSpecifiedLogLevel != null && userSpecifiedLogLevel != Level.OFF)) {
			...
			//
			if (userSpecifiedLogFile == null) {
				t4GlobalLogFile = T4LoggingUtilities
						.getUniqueLogFileName(T4LoggingUtilities.getUniqueID());
			} else {
			...
			//R3.1 changes -- start
			if(userSpecifiedSlowQueryLogFile == null){
				t4SlowQueryGlobalLogFile = T4LoggingUtilities.getUniqueLogFileName("QueryExecuteTime");
			}
			else{
				t4SlowQueryGlobalLogFile = userSpecifiedSlowQueryLogFile;
			}
			//R3.1 changes -- end
			...
			if (createGlobalLogFile == true) {
				...
				// KAS Degugging
				// System.out.println("In T4Properties.<static>");
				// System.out.println(" File = " + t4GlobalLogFile);
				// System.out.println(" Handler = " +
				// t4GlobalLogFileHandler.toString());
			...
			//R3.1 changes -- start
			if(createSlowQueryGlobalLogFile == true){
				...
				Formatter ff2 = new T4LogFormatter();
	...
	private String getProperty(String token) {
		...
		//system properties without the t4sqlmx prefix
		if (ret == null) {
	...
	private void setProperties() {
		...
		if (propsFile != null) {
			...
			if (propsFile.length() != 0) {
				...
				try {
					...
					fis = null;
					sqlExceptionMessage_ = "Error while loading " + propPrefix_
		...
		// property user. System properties must be prefixed with t4sqlmx.
		// For example user should be t4sqlmx.user if set as system property.
		...
		//R3.1 changes -- start
		setT4QueryExecuteLogFile(getProperty("T4QueryExecuteLogFile"));
		//R3.1 changes -- end
		setLogger(getHashTableEntry("logger"));
		setT4LogFileHandler(getHashTableEntry("T4LogFileHandler"));
		//R3.1 changes -- start
		setT4SlowQueryLogFileHandler(getHashTableEntry("T4SlowQueryLogFileHandler"));
		//R3.1 changes -- end
		...
		try {
			...
			sqlExceptionMessage_ = "Error while reading the clobTableName property: "
					+ se.getMessage();
		}
		...
		try {
			...
			sqlExceptionMessage_ = "Error while reading the blobTableName property: "
					+ se2.getMessage();
		}
		...
		setCommitInAutoCommitOn(getProperty("commitInAutoCommitOn"));
		...
		setKANJI(getProperty("KANJI"));
		setKSC5601(getProperty("KSC5601"));
		...
		setSeaquest(getProperty("seaquest"));
		setDelayedErrorMode(getProperty("delayedErrorMode"));
		//R3.0
		// Query Time out over-ride property
		// by default the querytimeout is DEFAULTED
		// specify if you dont want it to be ignored.
		if (getProperty("closeConnectionUponQueryTimeout") != null) {
			this.setCloseConnectionUponQueryTimeout(getProperty("closeConnectionUponQueryTimeout"));
		}
    //MFC new property enableMFC
    if(getProperty("enableMFC") != null){
    	this.setEnableMFC(getProperty("enableMFC"));
    }
    
    //property to support R2.3 date format Soln. 10-130717-8108
    if(getProperty("oldDateFormat") != null){
    	this.setOldDateFormat(getProperty("oldDateFormat"));
    }
		// RFE autoCommit changes
		String tempAutoCommit = null;
		tempAutoCommit = getProperty("autoCommit");
		if (tempAutoCommit != null) {
			this.setAutoCommit(tempAutoCommit);
		}
    
    //R3.1 changes - start
    if(getProperty("queryExecuteTime")!=null){
    	this.setQueryExecuteTime(getProperty("queryExecuteTime"));
    	
    }
    //R3.1 changes - end
     
  //R3.2 changes -- port from neo
    if(getProperty("keepAlive") != null){
    	this.setKeepAlive(getProperty("keepAlive"));
    }
    
    if(getProperty("executeBatchWithRowsAffected") != null){
    	this.setExecuteBatchWithRowsAffected(getProperty("executeBatchWithRowsAffected"));
    }

    if(getProperty("mfuStatementCache") != null) {
    	this.setMfuStatementCache(getProperty("mfuStatementCache"));
    }
	//10-090921-4730 -R30
		if (getProperty("externalCallPrefix") != null) {
			this.setExternalCallPrefix(getProperty("externalCallPrefix"));
		}

		if (getProperty("externalCallHandler") != null) {
			this.setExternalCallHandler(getProperty("externalCallHandler"));
		}

		//10-090921-4730
		
//	    if(getProperty("useExternalTransaction") != null) {
	    	this.setUseExternalTransaction(getProperty("useExternalTransaction"));
//		}
		//Soln 10-120724-4205
	    if(getProperty("dbMetaDataforDBTools") != null){
	    	setDbMetaDataforDBTools(getProperty("dbMetaDataforDBTools"));
	    }
	    
	    // Added for solution 10-190130-9260
	    if(getProperty("closeConnectionUponStatementCancel") != null) {
	        this.setCloseConnectionUponStatementCancel(getProperty("closeConnectionUponStatementCancel"));
	    }
	}
	...
	public Properties getProperties() {
		...
		//R3.1 changes -- start
		if(t4SlowQueryLogFileHandler_ != null){
			props.put("T4SlowQueryLogFileHandler", t4SlowQueryLogFileHandler_);
		}
		//R3.1 changes -- end
		...
		props.setProperty("connectionTimeout", String
				.valueOf(connectionTimeout_));
		...
		if (getClobTableName() != null) {
			...
			props.setProperty("KANJI", KANJI_);
		props.setProperty("KSC5601", KSC5601_);
		...
		props.setProperty("commitInAutoCommitOn", String
				.valueOf(commitInAutoCommitOn_));
		props.setProperty("SPJEnv", String.valueOf(SPJEnv_));
		props.setProperty("keepRawFetchBuffer", String
				.valueOf(keepRawFetchBuffer_));
		props.setProperty("cpuToUse", String.valueOf(cpuToUse_));
	//MFC new property enableMFC
    props.setProperty("enableMFC", this.enableMFC);
	//property to support R2.3 date format Soln. 10-130717-8108
	props.setProperty("oldDateFormat", this.oldDateFormat);
	//RFE autoCommit changes
	props.setProperty("autoCommit", this.autoCommit_);
   ...
   //R3.1 changes -- end
	...
	props.setProperty("mfuStatementCache", this.getMfuStatementCache());
		props.setProperty("replacementString", String
				.valueOf(replacementString_));
		...
		props.setProperty("seaquest", String.valueOf(_seaquest));
		props
				.setProperty("delayedErrorMode", String
						.valueOf(_delayedErrorMode));
		props.setProperty("closeConnectionUponQueryTimeout",
				this.closeConnectionUponQueryTimeout);
		...
		//Soln 10-120724-4205
		 ...
		 //Added for solution 10-190130-9260
		 props.setProperty("closeConnectionUponStatementCancel", this.getCloseConnectionUponStatementCancel());
		return props;
	}
	...
	void setDescription(String description) {
		if (description == null) {
			description_ = "NonStop JDBC T4 SQLMX DataSource.";
		} else {
	...
	/**
	 * Sets the data source name to use on the MXCS server side.
	 *
	 * @param serverDataSource
	 *            the data source name to use on the MXCS server side. The
	...
	/**
	 * Returns the MXCS server side data source name used for the current Type 4
	 * connection. The default server data source name is
	 * <code>TDM_Defaullt_DataSource</code>.
	 *
	 * @return MXCS server side data source Name.
	 ...
	 *
	 * @param catalog
	 *            NEO Database catalog name. The default catalog name is set by
	 *            the MXCS server side data source.
	 * @see #getCatalog()
	 */
	void setCatalog(String catalog) {
		...
		if (catalog_ != null) {
			...
			// Default catalog NEO - 1 6/27/2006
			// catalog_ = null;
			catalog_ = DEFAULT_CATALOG;
		}
	}
	 ...
	 *            by the MXCS server side data source.
	 * @see #getSchema()
	 */
	...
	void setMaxPoolSize(int maxPoolSize) {
		if (maxPoolSize < -1) {
			// KAS - Fix for Genesis case 10-040603-3501. The following line is
			// commented out.
	...
	void setInitialPoolSize(String initialPoolSize) {
		...
		if (initialPoolSize != null) {
			try {
				...
				sqlExceptionMessage_ = "Incorrect initialPoolSize value set: "
						+ initialPoolSize + ". " + ex.getMessage();
				initPs = -1;
			}
		}
	...
	void setMaxStatements(String maxStatements) {
		...
		if (maxStatements != null) {
			try {
				...
				sqlExceptionMessage_ = "Incorrect value for maxStatements set: "
						+ maxStatements + ex.getMessage();
				maxstmt = 0;
			}
		}
	...
	/**
	 * Returns the URL used in the current Type 4 connection. HP JDBC Type 4
	...
	/**
	 * Sets the URL for the Type 4 connection. HP JDBC Type 4 driver URL uses
	 * the following format:-
	 * <code>jdbc:t4sqlmx://host:port/:prop-name=value</code> This method
	...
	/**
	 * Validates the input url string follows the correct jdbc:t4sqlmx:<host>:<port>/:<prop-name=prop-val>;<prop-name=prop-val>
	 * format.
	 *
	 * @param url
	 ...
	 * Sets the URL for the Type 4 connection. HP JDBC Type 4 driver URL uses
	 * the following format:-
	 * <code>jdbc:t4sqlmx://host:port/:prop-name=value</code>. This
	...
	/**
	 * Sets the Safeguard user name to be used while connecting to MXCS server
	 * for authentication.
	 *
	 * @param user
	...
	void setLoginTimeout(String loginTimeout) {
		...
		if (loginTimeout != null) {
			try {
				...
				//start sol 10-080819-5288 R3.0
//				sqlExceptionMessage_ = "Incorrect value for loginTimeout set: "
//						+ loginTimeout + ex.getMessage();
				sqlExceptionMessage_ = "Provided LoginTimeout property value "+loginTimeout +
				" is invalid and should be less than Short.MAX_VALUE(32767)";

				//End sol 10-080819-5288 R3.0
				//loginTo = 1800;
				loginTo = 60;
			}
		}
	...
	void setLoginTimeout(int loginTimeout) {
		if (loginTimeout < 0) {
			sqlExceptionMessage_ = "Incorrect value for loginTimeout set: "
					+ loginTimeout + ".";
			//loginTimeout_ = 1800; changed to 60 secods R3.0
			loginTimeout_ = 60;
		} else {
	...
	void setNetworkTimeout(String networkTimeout) {
		...
		if (networkTimeout != null) {
			try {
				...
				sqlExceptionMessage_ = "Incorrect value for networkTimeout set: "
						+ networkTimeout + ex.getMessage();
				networkTo = 0;
			}
		}
	 ...
	 * value indicates the MXCS server to use the connection timeout value set
	 * by the administrator on the MXCS data source. @param connectionTimeout
	...
	void setConnectionTimeout(String connectionTimeout) {
		...
		if (connectionTimeout != null) {
			try {
				...
				sqlExceptionMessage_ = "Incorrect value for connectionTimeout set: "
						+ connectionTimeout + ". " + ex.getMessage();
				tmpTimeout = -1;
			}
		}
	...
	void setMaxIdleTime(String maxIdleTime) {
		...
		if (maxIdleTime != null) {
			try {
				...
				sqlExceptionMessage_ = "Incorrect value for maxIdleTime set: "
						+ maxIdleTime + ". " + ex.getMessage();
	...
	void setT4LogLevel(String level) {
		...
		if (level != null) {
			try {
				...
				SQLException se = SQLMXMessages.createSQLException(null, null,
						"problem_with_logging", ex.getMessage());
				sqlExceptionMessage_ = se.getMessage();
				// throw se;
	 ...
	 * @see java.util.logging.Level
	 */
	// Ravi to correct T4LogLevel with Jboss
	// 01'Feb'07 to make it for JBoss Logging for XA-datasource
	// Level getT4LogLevel() -R3.0
	String getT4LogLevel() {
		return t4LogLevel.toString();
	}
	 ...
	 * %h/t4sqlmx%u.log where: "/" represents the local pathname separator "%h"
	...
	void setT4QueryExecuteLogFile(String t4QueryExecuteLogFile) {
		if(t4QueryExecuteLogFile!=null){
			t4QueryExecuteLogFile_ = t4QueryExecuteLogFile;
		}
		else{
			t4QueryExecuteLogFile_=t4SlowQueryGlobalLogFile;
		}
		
	}

	String getT4QueryExecuteLogFile() {
		return t4QueryExecuteLogFile_;
	}
	//R3.1 changes -- End
		...
		try {
			if (t4LogFileHandler_ == null) {
				if (T4LogFile_.equals(t4GlobalLogFile)) {
					...
					//R3.1 changes for Soln 10-110701-8409  -- end
					t4LogFileHandler_ = new FileHandler(temp);
					t4LogFileHandler_.setFormatter(new T4LogFormatter());
				}
			}
		} catch (Exception e) {
			// KAS need to add error handling
			e.printStackTrace();
		}
	...
	FileHandler getT4SlowQueryLogFileHandler() {
		try {
			if (t4SlowQueryLogFileHandler_ == null) {
				if (t4QueryExecuteLogFile_.equals(t4SlowQueryGlobalLogFile)) {
					t4SlowQueryLogFileHandler_ = t4SlowQueryGlobalLogFileHandler;
				} else {
					t4SlowQueryLogFileHandler_ = new FileHandler(t4QueryExecuteLogFile_);
					t4SlowQueryLogFileHandler_.setFormatter(new T4LogFormatter());
				}
			}
		} catch (Exception e) {
			// KAS need to add error handling
		...
		return t4SlowQueryLogFileHandler_;
	} // end getT4LogFileHandler

	// --------------------------------------------
	void setT4SlowQueryLogFileHandler(Object fh) {
		t4SlowQueryLogFileHandler_ = (FileHandler) fh;
	}

	//R3.1 changes -- end
	
	
	// setter/getter methods for KANJI - Added for R3.0
	...
	//----------------------------------------------------------
	// setter/getter methods for KSC5601 - Added for R3.0
	...
	void setQueryTimeout(String queryTimeout) {
		...
		if (queryTimeout != null) {
			try {
				...
				sqlExceptionMessage_ = "Incorrect value for queryTimeout set: "
						+ queryTimeout + ex.getMessage();
				tmpQTimeOut = 0;
			}
		}
	...
	void setFetchBufferSize(String fetchBufferSize) {
		...
		if (fetchBufferSize != null) {
			try {
				...
				sqlExceptionMessage_ = "Incorrect fetchBufferSize value set: "
						+ setFetchSizeVal + ". " + ex.getMessage();
				setFetchSizeVal = 1;
			}
		}
	...
	/**
	 * Return whether the connection uses Nonstop SQLMX Database array binding feature.
	 *
	 * @return useArrayBinding boolean flag indicates Nonstop SQLMX Database RowSet (array
	 ...
	 * client classes and MXCS server on the NSK system.
	 *
	...
	/**
	 * Sets the buffer size in bytes used by the transport layer between Type 4
	 * client classes and MXCS server on the NSK system. Transport buffer
	 ...
	 * 4 client classes and MXCS server on the NSK system.
	 *
	...
	void setClobTableName(String clobTableName) throws SQLException {
		...
		if (clobTableName != null) {
			while (((fromIndex = clobTableName.indexOf('.', fromIndex + 1)) != -1)
					&& count < 2) {
				count++;
			}
			if (count < 2) {
	...
	void setBlobTableName(String blobTableName) throws SQLException {
		...
		if (blobTableName != null) {
			while (((fromIndex = blobTableName.indexOf('.', fromIndex + 1)) != -1)
					&& count < 2) {
				count++;
			}
			if (count < 2) {
	...
	void setReserveDataLocator(String reserveDataLocator) {
		...
		if (reserveDataLocator != null) {
			try {
				...
				sqlExceptionMessage_ = "Incorrect value for setReserveDataLocator set: "
						+ reserveDataLocator + ex.getMessage();
	...
	void setReserveDataLocator(long reserveDataLocatorLen) {
		if (reserveDataLocatorLen < 0) {
			sqlExceptionMessage_ = "Incorrect value for reserveDataLocator set: "
	...
	/**
	 * set "true" if it is OK to commit during when AutoCommit is ON.
	 *
	 * @return none
	 */
	void setCommitInAutoCommitOn(String p_commitInAutoCommitOn) {
		if ((p_commitInAutoCommitOn != null)
				&& (p_commitInAutoCommitOn.toUpperCase().equals("TRUE"))) {
			setCommitInAutoCommitOn(true);
		} else {
			setCommitInAutoCommitOn(false);
		}
	}
	/**
	 * set 'true' if it is OK to commit during when AutoCommit is ON.
	...
	void setCommitInAutoCommitOn(boolean p_commitInAutoCommitOn) {
		commitInAutoCommitOn_ = p_commitInAutoCommitOn;
	}
	/**
	 * returns 'true' if it is OK to commit during when AutoCommit is ON.
	 *
	 * @return true of false
	 */
	boolean getCommitInAutoCommitOn() {
		return commitInAutoCommitOn_;
	}
	...
	void setMxcsMajorVersion(String majorVer) {
		...
		if (majorVer != null) {
			try {
				...
				sqlExceptionMessage_ = "Incorrect value for MXCS major version set: "
						+ majorVer + ex.getMessage();
				mv = 0;
			}
		}
		setMxcsMajorVersion(mv);
		mxcsMajorVersion_ = mv;
	}

	short getMxcsMajorVersion() {
		return mxcsMajorVersion_;
	}

	// ----------------------------------------------------------
	void setMxcsMinorVersion(short minorVer) {
		mxcsMinorVersion_ = minorVer;
	}
	...
	void setMxcsMinorVersion(String minorVer) {
		...
		if (minorVer != null) {
			try {
				...
				sqlExceptionMessage_ = "Incorrect value for MXCS minor version set: "
						+ minorVer + ex.getMessage();
				mv = 0;
			}
		}
		setMxcsMinorVersion(mv);
		mxcsMinorVersion_ = mv;
	}

	short getMxcsMinorVersion() {
		// return (short)3; // KAS debugging
		return mxcsMinorVersion_;
	}
	...
	void setSqlmxMajorVersion(String majorVer) {
		...
		if (majorVer != null) {
			try {
				...
				sqlExceptionMessage_ = "Incorrect value for SQL/MX major version set: "
						+ majorVer + ex.getMessage();
				mv = 0;
			}
		}
	...
	//soln 10-130828-8563 changes 
	public String getISO88591() {
		return ISO88591_;
	}
	public void setISO88591(String lang) {
		if (lang == null) {
			ISO88591_ = InterfaceUtilities
					.getCharsetName(InterfaceUtilities.SQLCHARSETCODE_ISO88591);
		} else {
			ISO88591_ = lang;
		}
	}
	...
	void setSeaquest(String seaquest) {
		setSeaquest(Boolean.valueOf(seaquest).booleanValue());
	}
	...
	void setSqlmxMinorVersion(String minorVer) {
		...
		if (minorVer != null) {
			try {
				...
				sqlExceptionMessage_ = "Incorrect value for SQL/MX minor version set: "
						+ minorVer + ex.getMessage();
				mv = 0;
			}
		}
	...
	BigDecimal getMxcsVersion() {
		String minor = new Short(getMxcsMinorVersion()).toString();
		String major = new Short(getMxcsMajorVersion()).toString();
	...
	Reference addReferences(Reference ref) {
		...
		ref.add(new StringRefAddr("connectionTimeout", Integer
				.toString(getConnectionTimeout())));
		ref.add(new StringRefAddr("maxIdleTime", Integer
				.toString(getMaxIdleTime())));
		ref.add(new StringRefAddr("loginTimeout", Integer
				.toString(getLoginTimeout())));
		// ref.add(new StringRefAddr("closeConnectionTimeout",
		// Integer.toString(getCloseConnectionTimeout())));
		ref.add(new StringRefAddr("networkTimeout", Integer
				.toString(getNetworkTimeout())));
		ref.add(new StringRefAddr("T4LogLevel", getT4LogLevel().toString()));
		ref.add(new StringRefAddr("T4LogFile", getT4LogFile()));
		//R3.1 changes -- start
		ref.add(new StringRefAddr("T4QueryExecuteLogFile",getT4QueryExecuteLogFile()));
		ref.add(new StringRefAddr("queryExecuteTime", Long.toString(getQueryExecuteTime())));
		//R3.1 changes -- end		
		ref.add(new StringRefAddr("KANJI", getKANJI()));
		ref.add(new StringRefAddr("KSC5601", getKSC5601()));
		...
		ref.add(new StringRefAddr("reserveDataLocator", Long
				.toString(reserveDataLocator_)));
		ref.add(new StringRefAddr("roundingMode", Integer
				.toString(getRoundingMode())));

		// propertiy queryTimeout_ for future use.
		ref.add(new StringRefAddr("queryTimeout", Integer
				.toString(getQueryTimeout())));
		ref.add(new StringRefAddr("fetchBufferSize", Short.toString(this
				.getFetchBufferSize())));
		ref.add(new StringRefAddr("batchRecovery", Boolean.toString(this
				.getBatchRecovery())));
		//Added for the solution 10-160414-9785 
		ref.add(new StringRefAddr("ISO88591", getISO88591()));
		//Added for solution 10-190130-9260
		ref.add(new StringRefAddr("closeConnectionUponStatementCancel", getCloseConnectionUponStatementCancel()));
		return ref;
	}
	 ...
	 */
	DriverPropertyInfo[] getPropertyInfo(String url, Properties defaults)
			throws SQLException {

		String level[] = { "OFF", "SEVERE", "WARNING", "INFO", "CONFIG",
				"FINE", "FINER", "FINEST" };
		String roundingMode[] = { "ROUND_CEILING", "ROUND_DOWN", "ROUND_UP",
				"ROUND_FLOOR", "ROUND_HALF_UP", "ROUND_UNNECESSARY",
				"ROUND_HALF_EVEN", "ROUND_HALF_DOWN", "ROUND_DOWN" };

String closeConnectionUponQueryTimeoutValues[] = { "DEFAULT", "IGNORE" };
    //MFC property new enableMFC
    String enableMFC [] = {"ON","OFF"};
    //RFE autoCommit changes
    String autoCommit_ [] = {"ON","OFF"};
	//property to support R2.3 date format Soln. 10-130717-8108
	String oldDateFormat [] = {"ON","OFF"};
    //R3.2 changes -- port from neo
    String keepAlive [] = {"ON","OFF"};
    //Soln 10-120724-4205
    String dbMetaDataforDBTools [] = {"OFF","ON"};
    
    String executeBatchWithRowsAffected [] = {"ON","OFF"};
	String mfuStatementCache [] = {"YES", "NO"};
	String useExternalTransaction [] = {"YES", "NO"};
		...
		DriverPropertyInfo[] propertyInfo = {
				setPropertyInfo("catalog", props, true,
						"SQL/MX2.0 catalog name", null),
				setPropertyInfo("schema", props, true, "SQL/MX2.0 schema name",
						null),
				setPropertyInfo("url", props, false,
						"jdbc:t4sqlmx://<host>:<port>/:", null),
				setPropertyInfo("user", props, true, "NSK safeguard user name",
						null),
				setPropertyInfo("password", props, true,
						"NSK safeguard user password", null),
				setPropertyInfo("maxPoolSize", props, false,
						"Maximum connection pool size", null),
				setPropertyInfo("minPoolSize", props, false,
						"Minimum connection pool size", null),
				setPropertyInfo("initialPoolSize", props, false,
						"Initial connection pool size", null),
				setPropertyInfo("maxStatements", props, false,
						"Maximum statement pool size", null),
				setPropertyInfo("T4LogLevel", props, false, "Logging Level",
						level),
				setPropertyInfo("T4LogFile", props, false,
						"Logging file location", null),
				setPropertyInfo("loginTimeout", props, false,
						"Login time out in secs", null),
				setPropertyInfo("networkTimeout", props, false,
						"Network time out in secs", null),
				setPropertyInfo("connectionTimeout", props, false,
						"Connection time out in secs", null),
				setPropertyInfo("maxIdleTime", props, false,
						"Max idle time for a free pool connection in secs",
						null),
				setPropertyInfo("language", props, false,
						"Locale language to use", null),
				setPropertyInfo("serverDataSource", props, false,
						"MXCS data source name", null),
				setPropertyInfo("roundingMode", props, false,
						"Data rounding mode", roundingMode),
				setPropertyInfo("KANJI", props, false,
						"KANJI column character set translation", null),
				setPropertyInfo("KSC5601", props, false,
						"KSC5601 column character set translation", null),
				...
				setPropertyInfo(
						...
						props,
						false,
						"Number of data locators (for LOB) to be reserved by the connection",
						null),
				setPropertyInfo(
								"closeConnectionUponQueryTimeout",
								props,
								false,
								"Doesnt throw unsupported feature exception when the value is IGNORE",
								closeConnectionUponQueryTimeoutValues),
				setPropertyInfo(
						"fetchBufferSize",
						props,
						false,
						"Value (in KB) for the size of the fetch buffer to be used when rows are fetched",
						null),

				setPropertyInfo("enableMFC",
						props,
						false,
						"Use MFC for Statement Cache enable",
						enableMFC),
				//property to support R2.3 date format Soln. 10-130717-8108
				setPropertyInfo("oldDateFormat",
						props,
						false,
						"Used to support old date format",
						oldDateFormat),
						//R3.1 changes - start
				setPropertyInfo("queryExecuteTime",
								props,
								false,
								"Used to Idenfify queries exectution time",
								null),
								
				setPropertyInfo("T4QueryExecuteLogFile",
								props,
								false,
								" Log file for execute queries",
								null),			
						//R3.1 changes - end	
								
								//R3.2 changes -- port from neo
								setPropertyInfo("keepAlive",
										props,
										false,
										"Use to keep connection alive",
										keepAlive),
										
				setPropertyInfo("executeBatchWithRowsAffected",
						props,
						false,
						"To Return valid rowsaffected result for executebatch",
				...
				setPropertyInfo(
						...
						props,
						false,
						"Changes the Driver Side Statement Caching Strategy to Most Frequently used, the default is Most Recently used",
						mfuStatementCache),
				setPropertyInfo(
						"batchRecovery",
						props,
						false,
						"Continue batch processing of next commands even after errors",
						null),
				setPropertyInfo(
						"useExternalTransaction",
						props,
						false,
						"Inherits the transaction started by application, to use for DB operations ",
						useExternalTransaction),
						//Soln 10-120724-4205
				setPropertyInfo("dbMetaDataforDBTools",props,false,"Property for changing the behaviour of DatabaseMetaData.getSchemas()and getCatalog() to assist DB tools",
						dbMetaDataforDBTools),
						//RFE autoCommit changes
		        setPropertyInfo("autoCommit",
								props,
								false,
								"Use this property to Turn ON/OFF autoCommit mode",
								autoCommit_)
		        		};
	...
	private DriverPropertyInfo setPropertyInfo(String name, Properties props,
			boolean required, String description, String[] choices) {
	...
	public String getCloseConnectionUponQueryTimeout() {
		return closeConnectionUponQueryTimeout;
	}
	public void setCloseConnectionUponQueryTimeout(
			String closeConnectionUponQueryTimeout) {
		if (closeConnectionUponQueryTimeout.equals("DEFAULT")) {
			this.closeConnectionUponQueryTimeout = SQLMXSwitchConCloseOnQueryTimeout.DEFAULT;
		} else if (closeConnectionUponQueryTimeout.equals("IGNORE")) {
			this.closeConnectionUponQueryTimeout = SQLMXSwitchConCloseOnQueryTimeout.IGNORE;
		} else if (closeConnectionUponQueryTimeout.equals("ON")) {
			this.closeConnectionUponQueryTimeout = SQLMXSwitchConCloseOnQueryTimeout.ON;
		} else {
			this.closeConnectionUponQueryTimeout = SQLMXSwitchConCloseOnQueryTimeout.DEFAULT;
		}
	}
...
//MFC new property enableMFC R3.0
	...
	public void setAutoCommit(String autoCommit) {
		if(autoCommit.equalsIgnoreCase("ON")|| autoCommit.equalsIgnoreCase("OFF")){
		this.autoCommit_ = autoCommit.toUpperCase();
		}
	}
	...
	// Added property to support R2.3 date format Soln. 10-130717-8108
	 ...
	 public void setOldDateFormat(String oldDateFormat) {
		if(oldDateFormat.equalsIgnoreCase("ON") || oldDateFormat.equalsIgnoreCase("OFF")) {
            this.oldDateFormat = oldDateFormat.toUpperCase();
        }
	 }
	...
	public void setKeepAlive(String keepAlive) {
		if (keepAlive.equalsIgnoreCase("ON")
				|| keepAlive.equalsIgnoreCase("OFF")) {
	...
	void setQueryExecuteTime(String queryExecuteTime) {
		long tmpQueryExecuteTime = 0;
		if (queryExecuteTime != null) {
			try {
				tmpQueryExecuteTime = Long.parseLong(queryExecuteTime);
			} catch (NumberFormatException ex) {
				sqlExceptionMessage_ = "Incorrect value for queryExecuteTime set: "
						+ queryExecuteTime + ex.getMessage();
				tmpQueryExecuteTime = 0;
			}
		}
		setQueryExecuteTime(tmpQueryExecuteTime);
	}

	//R3.1 changes - end
	
	public String getExecuteBatchWithRowsAffected() {
		return executeBatchWithRowsAffected;
	}
	...
	public String getUseExternalTransaction() {
		return useExternalTransaction;
	}


	public void setUseExternalTransaction(String useExternalTransaction) {
		if (useExternalTransaction == null){
			this.useExternalTransaction = "NO";
		}else {
		this.useExternalTransaction = useExternalTransaction;
		}
	}
	//10-090921-4730
// Soln 10-120724-4205
	public String getDbMetaDataforDBTools () {
		return dbMetaDataforDBTools ;
	}


	public void setDbMetaDataforDBTools (
			String dbMetaDataforDBTools ) {
	...
	//Added for solution 10-190130-9260
	public String getCloseConnectionUponStatementCancel() {
        return this.closeConnectionUponStatementCancel;
    }

    public void setCloseConnectionUponStatementCancel(String closeConnectionUponStatementCancel) {
        if(closeConnectionUponStatementCancel.equalsIgnoreCase("NO")) {
            this.closeConnectionUponStatementCancel = closeConnectionUponStatementCancel.toUpperCase();
        }
    }
}
