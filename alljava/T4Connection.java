...
class T4Connection {
	...
	static final long INCONTEXT_OPT1_SESSIONNAME = 0x80000000;

	static final long INCONTEXT_OPT1_FETCHAHEAD = 1073741824; // (2^30)
	...
	T4Connection(InterfaceConnection ic) throws SQLException {
		...
		getInputOutput().setTimeout(ic.getLoginTimeout()*1000);
		//End
	...
	private void checkConnectionIdleTimeout() throws SQLException {
		if (m_io.checkConnectionIdleTimeout()) {
			...
			throw SQLMXMessages.createSQLException(m_ic.t4props_, m_locale,
					"ids_s1_t00", null);
		}
	}
	...
	protected void throwInternalException() throws SQLException {
		...
		SQLException se = SQLMXMessages.createSQLException(tempP, m_locale,
				"internal_error", null);
		SQLException se2 = SQLMXMessages.createSQLException(tempP, m_locale,
	...
	protected LogicalByteArray getReadBuffer(short odbcAPI,
		...
		if (m_ic.t4props_.t4Logger_.isLoggable(Level.FINEST) == true) {
			Object p[] = T4LoggingUtilities.makeParams(m_ic.t4props_,m_ic);
			String temp = "LogicalByteArray";
			m_ic.t4props_.t4Logger_.logp(Level.FINEST, "T4Connection",
					"getReadBuffer", temp, p);
		}
	...
	InitializeDialogueReply InitializeDialogue() throws SQLException {
		try {
			...
			if (this.m_ic.t4props_.getFetchAhead()) {
				optionFlags1 |= this.INCONTEXT_OPT1_FETCHAHEAD;
			}
			...
			if (m_ic.t4props_.t4Logger_.isLoggable(Level.FINEST) == true) {
				...
				m_ic.t4props_.t4Logger_.logp(Level.FINEST, "T4Connection",
						"InitializeDialogue", temp, p);
			}
			LogicalByteArray wbuffer = InitializeDialogueMessage.marshal(
					m_userDesc, m_inContext, m_dialogueId, optionFlags1,
					optionFlags2, m_sessionName, m_ic);
			//Modification for L36 Corda : converting to milliseconds
			getInputOutput().setTimeout(m_ic.t4props_.getLoginTimeout()*1000);
			//End
			LogicalByteArray rbuffer = getReadBuffer(
					TRANSPORT.SRVR_API_SQLCONNECT, wbuffer);

			//
			// Process output parameters
			//
			InitializeDialogueReply idr1 = new InitializeDialogueReply(rbuffer,
					m_ncsAddress.getIPorName(), m_ic);
			...
			SQLException se = SQLMXMessages.createSQLException(m_ic.t4props_,
					...
					"InitializeDialogueMessage", e.getMessage());
			se.initCause(e);
			throw se;
			...
			SQLException se = SQLMXMessages.createSQLException(m_ic.t4props_,
					m_locale, "initialize_dialogue_message_error", e
							.getMessage());
	...
	TerminateDialogueReply TerminateDialogue() throws SQLException {
		try {
			...
			if (m_ic.t4props_.t4Logger_.isLoggable(Level.FINEST) == true) {
				...
				String temp = "m_dialogueId=" + m_dialogueId;
				m_ic.t4props_.t4Logger_.logp(Level.FINEST, "T4Connection",
						"TerminateDialogue", temp, p);
			}
			LogicalByteArray wbuffer = TerminateDialogueMessage.marshal(
					m_dialogueId, this.m_ic);

			//
			// KAS: I use m_ic instead of getInputOutput, because getInputOutput
			...
			//Modification for L36 Corda : converting to milliseconds
			m_io.setTimeout(m_ic.t4props_.getLoginTimeout()*1000);
			//End
			LogicalByteArray rbuffer = getReadBuffer(
					TRANSPORT.SRVR_API_SQLDISCONNECT, wbuffer);

			//
			// Process output parameters
			//
			TerminateDialogueReply tdr1 = new TerminateDialogueReply(rbuffer,
					m_ncsAddress.getIPorName(), m_ic);
			...
			SQLException se = SQLMXMessages.createSQLException(m_ic.t4props_,
					m_locale, "translation_of_parameter_failed",
					"TerminateDialogMessage", e.getMessage());
			se.initCause(e);
			throw se;
		} catch (UnsupportedCharsetException e) {
			SQLException se = SQLMXMessages.createSQLException(m_ic.t4props_,
			...
			SQLException se = SQLMXMessages.createSQLException(m_ic.t4props_,
					m_locale, "terminate_dialogue_message_error", e
							.getMessage());
	 ...
	 *                SQLException is thrown
	 */
	SetConnectionOptionReply SetConnectionOption(short connectionOption,
			int optionValueNum, String optionValueStr) throws SQLException {
		...
		try {
			...
			LogicalByteArray wbuffer = SetConnectionOptionMessage.marshal(
					m_dialogueId, connectionOption, optionValueNum,
					optionValueStr, this.m_ic);
			//Modification for L36 Corda : NetworkTimeout in milliseconds
			getInputOutput().setTimeout(m_ic.getNetworkTimeout());
			//End
			LogicalByteArray rbuffer = getReadBuffer(
					TRANSPORT.SRVR_API_SQLSETCONNECTATTR, wbuffer);

			SetConnectionOptionReply scor = new SetConnectionOptionReply(
					rbuffer, m_ncsAddress.getIPorName(), m_ic);
			...
			SQLException se = SQLMXMessages.createSQLException(m_ic.t4props_,
					m_locale, "translation_of_parameter_failed",
					"SetConnectionOptionReply", e.getMessage());
			se.initCause(e);
			throw se;
		} catch (UnsupportedCharsetException e) {
			SQLException se = SQLMXMessages.createSQLException(m_ic.t4props_,
			...
			SQLException se = SQLMXMessages.createSQLException(m_ic.t4props_,
					m_locale, "set_connection_option_message_error", e
							.getMessage());
	 ...
	 *                SQLException is thrown
	 */
	EndTransactionReply EndTransaction(short transactionOpt)
			throws SQLException {

		try {
			LogicalByteArray wbuffer = EndTransactionMessage.marshal(
					m_dialogueId, transactionOpt, this.m_ic);
			//Modification for L36 Corda : NetworkTimeout in milliseconds
			getInputOutput().setTimeout(m_ic.getNetworkTimeout());
			//End
			LogicalByteArray rbuffer = getReadBuffer(
					TRANSPORT.SRVR_API_SQLENDTRAN, wbuffer);

			EndTransactionReply cr = new EndTransactionReply(rbuffer,
					m_ncsAddress.getIPorName(), m_ic);
			return cr;
		} // end try
		catch (SQLException se) {
			...
			SQLException se = SQLMXMessages.createSQLException(m_ic.t4props_,
					m_locale, "translation_of_parameter_failed",
					"EndTransactionMessage", e.getMessage());
			se.initCause(e);
			throw se;
		} catch (UnsupportedCharsetException e) {
			SQLException se = SQLMXMessages.createSQLException(m_ic.t4props_,
			...
			SQLException se = SQLMXMessages.createSQLException(m_ic.t4props_,
					m_locale, "end_transaction_message_error", e.getMessage());
	 ...
	 *                SQLException is thrown
	 */
	GetSQLCatalogsReply GetSQLCatalogs(String stmtLabel, short APIType,
		...
		try {
			...
			wbuffer = GetSQLCatalogsMessage.marshal(m_dialogueId, stmtLabel,
					APIType, catalogNm, schemaNm, tableNm, tableTypeList,
			...
			//End
			LogicalByteArray rbuffer = getReadBuffer(
					TRANSPORT.SRVR_API_GETCATALOGS, wbuffer);

			//
			// Process output parameters
			//
			GetSQLCatalogsReply gscr = new GetSQLCatalogsReply(rbuffer,
					m_ncsAddress.getIPorName(), m_ic);
			...
			SQLException se = SQLMXMessages.createSQLException(m_ic.t4props_,
					m_locale, "translation_of_parameter_failed",
					"GetSQLCatalogsMessage", e.getMessage());
			se.initCause(e);
			throw se;
		} catch (UnsupportedCharsetException e) {
			SQLException se = SQLMXMessages.createSQLException(m_ic.t4props_,
			...
			SQLException se = SQLMXMessages.createSQLException(m_ic.t4props_,
					m_locale, "get_sql_catalogs_message_error", e.getMessage());
