...
class Utility {
	...
	private static final byte key[] = Utility.UnicodeToAscii("ci4mg04-3;"
			+ "b,hl;y'd1q" + "x8ngp93nGp" + "oOp4HlD7vm" + ">o(fHoPdkd"
			+ "khp1`gl0hg" + "qERIFdlIFl" + "w48fgljksg" + "3oi5980rfd"
			+ "4t8u9dfvkl");
	 ...
	 *
	 */
	static void checkFloatBoundary(Locale locale, BigDecimal inbd)
			throws SQLException {
		...
		if (indbl > (double) Float.MAX_VALUE) {
			throw SQLMXMessages.createSQLException(null, locale,
					"numeric_out_of_range", inbd.toString());
		}
	 ...
	 *
	 */
	static void checkDoubleBoundary(Locale locale, BigDecimal inbd)
			throws SQLException {
	 ...
	 *
	 */
	static void checkIntegerBoundary(Locale locale, BigDecimal inbd)
			throws SQLException {
	 ...
	 *
	 */
	static void checkSignedLongBoundary(Locale locale, BigDecimal inbd)
			throws SQLException {
		long inlong = inbd.longValue();
	 ...
	 *
	 */
	static void checkSignedShortBoundary(Locale locale, BigDecimal inbd)
			throws SQLException {
		long inlong = inbd.longValue();
	 ...
	 *
	 */
	static void checkUnsignedIntegerBoundary(Locale locale, BigDecimal inbd)
			throws SQLException {
		long inlong = inbd.longValue();
	 ...
	 *
	 */
	static void checkTinyintBoundary(Locale locale, BigDecimal inbd)
			throws SQLException {
		long inlong = inbd.longValue();
	 ...
	 *
	 */
	static void checkShortBoundary(Locale locale, BigDecimal inbd)
			throws SQLException {
		long inlong = inbd.longValue();
	 ...
	 *
	 */
	static BigDecimal getBigDecimalValue(Locale locale, Object paramValue)
			throws SQLException {
		BigDecimal tmpbd;
		...
		if (paramValue instanceof Long) {
			...
			tmpbd = BigDecimal
					.valueOf(((((Boolean) paramValue).booleanValue() == true) ? 1
							: 0));
			...
			throw SQLMXMessages.createSQLException(null, locale,
					"object_type_not_supported", paramValue);
		}
		return tmpbd;
	 ...
	 *
	 */
	static void checkDecimalBoundary(Locale locale, BigDecimal inbd,
	 ...
	 *
	 */
	static void checkDecimalTruncation(int parameterIndex, Locale locale,
			BigDecimal inbd, int precision, int scale) throws SQLException {
		if (precision <= 0)
			return;
		...
		// SOLN-8432 AM 08/17/2006
		actualLen = inbd.toString().length();
		if (precision > 0) {
			//Soln. 10-130919-3915 JDBCT4: DataTruncation exception occurs when inserting the maximum value into NU 
			Double db = Math.pow(10, precision - scale);
			BigDecimal maxbd = new BigDecimal(Double.toString(db));
		//	BigDecimal maxbd = new BigDecimal(Math.pow(10, precision - scale));
			//end Soln. 10-130919-3915
			BigDecimal minbd = maxbd.negate();
			if ((inbd.compareTo(maxbd) >= 0) || (inbd.compareTo(minbd) < 0)) {
				// SOLN-8432 AM 08/17/2006
				...
				DataTruncation dt = new DataTruncation(parameterIndex, true,
						false, actualLen, expectedLen);
				dt.setNextException(new SQLException("DataTruncation", "22003",
						-8411));
				throw dt;
			}
		}
	 ...
	 *
	 */
	static void checkLongBoundary(Locale locale, BigDecimal inbd)
			throws SQLException {
		if ((inbd.compareTo(long_maxbd) > 0)
				|| (inbd.compareTo(long_minbd) < 0)) {
			throw SQLMXMessages.createSQLException(null, locale,
					"numeric_out_of_range", inbd.toString());
		}
	} // end checkBigDecimalBoundary
	 ...
	 *
	 */
	static void checkLongTruncation(int parameterindex, BigDecimal inbd)
			throws SQLException {
		long inlong = inbd.longValue();
		double indbl = inbd.doubleValue();
	...
	static int getRoundingMode(int roundMode) {
		if ((roundMode == BigDecimal.ROUND_CEILING)
				...
				|| (roundMode == BigDecimal.ROUND_UNNECESSARY)
				|| (roundMode == BigDecimal.ROUND_HALF_EVEN)
				|| (roundMode == BigDecimal.ROUND_HALF_DOWN)) {
			return roundMode;
		} else {
			//Soln 10-091208-6912 -- start R30
//			return BigDecimal.ROUND_DOWN;
	...
	static BigDecimal setScale(BigDecimal tmpbd, int scale, int roundingMode)
			throws SQLException {
		try {
			if (scale > -1) {
