...
class SQLMXDesc {
	...
	String getColumnClassName() throws SQLException {
		...
		/*BINARY data type is added in SQL/MX 3.6*/
		case Types.BINARY:
	...
	String getColumnTypeName(Locale locale) throws SQLException {
		...
		case Types.LONGVARCHAR:
			return "LONG VARCHAR";
		//R3.6 Addded Binary and VarBinary Data types
		case Types.BINARY:
			return "BINARY";
		case Types.VARBINARY:
			return "VARBINARY";
		...
		case Types.OTHER:
			...
			// Star sol. 10-090915-4592 R3.0
			else if (sqlDataType_ == InterfaceResultSet.SQLTYPECODE_DATETIME) {
				return "DATETIME";
			}
			// End sol. 10-090915-4592 R3.0
			else {
				return "UNKNOWN";
			}
		case Types.BIT:
	...
	SQLMXDesc(int dataType, short datetimeCode, int maxLen, short precision,
			short scale, boolean nullInfo, String colName, boolean signType,
			int odbcDataType, short odbcPrecision, int sqlCharset,
			int odbcCharset, String catalogName, String schemaName,
			String tableName, int fsDataType, int intLeadPrec, int paramMode) {
		...
		if (sqlDataType_ == InterfaceResultSet.SQLTYPECODE_DATETIME) // 9
		{
			...
			case 11: // ODBC2 TIMESTAMP
				...
				case SQLDTCODE_HOUR_TO_FRACTION: // note: Nonstop SQL/MX Database
					// maps to TIME(6)
		...
		case Types.TIMESTAMP:
			sqlOctetLength_ = maxLen + 3;
			/*
			 * Commented out by SB on 6/28/2005 After discussion with Rao and
			 * looking at the server code, we realize that the odbcPrecision
			 * value takes care of the following logic. Hence we should depend
			 * on the server for the precision rather than coding our own.
			 * precision_ = precision; // Should come for Server - Must be
			 * changed if (precision_ == 0) { displaySize_ = 19; } else {
			 * displaySize_ = 20 + precision_; } precision_ = 23; // ODBC2.0
			 */
			precision_ = odbcPrecision;
			displaySize_ = precision_;
			if (sqlDatetimeCode_ > 3 && sqlDatetimeCode_!= InterfaceResultSet.SQLDTCODE_DC_DATETIME) // if it is more than 3, it is one of
		...
		case Types.CHAR:
			...
			// I'm removing this add 1 and the subtract 1 in InterfaceStatement.
			// After
			// talking with Velu, it sounds like this is a bug that was picked
			// up from
			// the type 2 driver. 2/1/2005 Ken.
		...
		/*BINARY data type is added in SQL/MX 3.6*/
		case Types.BINARY:
			if(sqlDataType_ == InterfaceResultSet.SQLTYPECODE_BINARY)
				dataType_ = Types.BINARY;
			sqlOctetLength_ = maxLen;
			displaySize_ = maxLen;
			precision_ = maxLen; // ODBC2.0
			break;
		...
		case Types.VARBINARY:
			if(sqlDataType_ == InterfaceResultSet.SQLTYPECODE_VARBINARY_WITH_LENGTH)
				dataType_ = Types.VARBINARY;
			sqlOctetLength_ = maxLen + 3;
			displaySize_ = maxLen;
			precision_ = maxLen; // ODBC2.0
			break;
		...
		if (sqlDataType_ == SQLTYPECODE_CHAR
				|| sqlDataType_ == SQLTYPECODE_VARCHAR
				|| sqlDataType_ == SQLTYPECODE_VARCHAR_LONG
				|| sqlDataType_ == InterfaceResultSet.SQLTYPECODE_VARCHAR2_WITH_LENGTH /* VARCHAR2 changes for SQL/MX3.5 */
	...
	SQLMXDesc(
			...
			, int dataType, short datetimeCode, int maxLen, short precision,
			short scale,
			boolean nullInfo,
			...
			// as
			// colHeadingNm
			, String tableName, String catalogName, String schemaName,
			String headingName // Descriptor2
			// only
		...
		//
		this(dataType, datetimeCode, maxLen, precision, scale, nullInfo,
				colName, signType, odbcDataType, (short) odbcPrecision,
				sqlCharset, odbcCharset, catalogName, schemaName, tableName,
				fsDataType, intLeadPrec, paramMode);
		//
		...
		//Added for solution 10-110518-7717
		columnLabel_ = headingName;
		maxLen_ = maxLen;
	...
	// ---------------------------------------------------------------
	/***************************************************************************
	 * Returns encoding type for character data types from Nonstop SQL/MX Database
	 * COLS table.
	 */
	...
	// public static final int SQLDTCODE_YEAR_TO_DAY 1 //Nonstop SQL/MX Database
	// DATE
	public static final int SQLDTCODE_YEAR_TO_HOUR = 7; // ODBC TIMESTAMP(0)

	public static final int SQLDTCODE_YEAR_TO_MINUTE = 8;

	// public static final int SQLDTCODE_YEAR_TO_SECOND 3 //Nonstop SQL/MX
	// DatabaseTIMESTAMP(0)
	// public static final int SQLDTCODE_YEAR_TO_FRACTION 3 //Nonstop SQL/MX
	...
	// define SQLDTCODE_HOUR_TO_SECOND 2 //Nonstop SQL/MX Database TIME(0) --> NCS
	// Maps this to TIME
	public static final int SQLDTCODE_HOUR_TO_FRACTION = 2; // Nonstop SQL/MX
	...
	// parameter
	// For LOB Support - Changed to Object SB 10/8/2004
