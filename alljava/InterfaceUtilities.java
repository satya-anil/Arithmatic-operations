...
public class InterfaceUtilities {
	...
	static {
		...
		valueToCharset.put(new Integer(-1), "KANJI"); // KANJI -- Added for R3.0
		valueToCharset.put(new Integer(-2), "KSC5601"); // KSC5601 -- Added for R3.0
	}
	...
	static {
		...
		charsetToValue.put("KANJI", new Integer(-1));  //KANJI -- Added for R3.0
		charsetToValue.put("KSC5601",new Integer(-2)); //KSC5601 -- Added for R3.0
	...
	static String getEncodingTranslation(SQLMXConnection conn, int SQLcharset) {
		...
		if (SQLcharset == SQLCHARSETCODE_ISO88591) {
			rValue = conn.props_.getISO88591();
		} else if (SQLcharset == SQLCHARSETCODE_KANJI) {
			rValue = conn.props_.getKANJI();
		} else if (SQLcharset == SQLCHARSETCODE_KSC5601) {
			rValue = conn.props_.getKSC5601();
		} else if (SQLcharset == SQLCHARSETCODE_UNICODE) { 
//			rValue = InterfaceUtilities.SQLCHARSET_UNICODE;
			rValue = "UnicodeBigUnmarked";
		} else {
			rValue = SQLCHARSET_UNKNOWN;
		}

		return rValue;
	} // end getEncodingTranslation
	//end
	
	public static byte[] convertBigDecimalToSQLBigNum(BigDecimal bd,
			int targetLength, int targetScale) {
		byte[] sourceData = bd.setScale(targetScale, BigDecimal.ROUND_DOWN)
				.unscaledValue().toString().getBytes(); // add
		// trailing
		// 0s,
	...
	public static BigDecimal convertSQLBigNumToBigDecimal(byte[] sourceData,
			int scale) {
		...
		for (int i = 0; i < dataInShorts.length; i++)
			dataInShorts[i] = Bytes.extractUShort(sourceData, i * 2, false); // copy
		// the
		// data
