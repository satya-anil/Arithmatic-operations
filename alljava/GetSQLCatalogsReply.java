...
class GetSQLCatalogsReply {
	...
	// -------------------------------------------------------------
	GetSQLCatalogsReply(LogicalByteArray buf, String addr,
		...
		if (m_p1.exception_nr == TRANSPORT.CEE_SUCCESS) {
			m_p2 = ic.decodeBytes(buf.extractString(), 1);
			...
			proxySyntax = ic.decodeBytes(buf.extractString(),
					InterfaceUtilities.SQLCHARSETCODE_UTF8);
		}
	}
}
